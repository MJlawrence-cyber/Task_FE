...
try {
    ...
    toast.error('error while getting user tasks');
    throw error;
}
  });
...
export const fetchSingleTask = createAsyncThunk('tasks/fetchSingleTask', async (taskId) => {
try {
    const response = await api.getWithOutToken(`/tasks/get${taskId}`);
    return response;
} catch (error) {
    toast.error('could not fetch single task');
    throw error;
}
});
...
export const deleteTask = createAsyncThunk("tasks/deleteTask", async (taskId) => {
  ...
  export const updateTask = createAsyncThunk("tasks/updateTask", async (taskData) => {
    ...
    try {
      const response = await api.putWithToken(`/tasks/edit/${taskId}`, updatedData);
      return response;
    } catch (error) {
